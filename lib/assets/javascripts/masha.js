...
MaSha.prototype = {
    ...
    *
    ...
    getSelection: function(){
        ...
        return {text: this.currentRange.toString(), location: this.serializeRange(this.currentRange)};
    },
    ...
    *
    ...
    addSelection: function(range) {
        ...
        return this.addAnchorForSelection(range, class_name);
    },

    /*
    * Adds anchor to a range selection, and returns the anchor dom element
    *
     */
    addAnchorForSelection: function(range, class_name) {
        var anchor_span = document.createElement('span');
        anchor_span.className = 'anchor' + ' ' + class_name;

        range.addAnchor(anchor_span);
    ...
    *
    ...
    highlightSelection: function(class_name){
        if (! this.ranges[class_name]) return;
        range = this.deserializeRange(this.ranges[class_name]);
        if (! range) return;

        range.wrapSelection(class_name);
    ...
    unhighlightSelection: function(class_name){
        ...
        range = this.deserializeRange(this.ranges[class_name]);
        if (! range) return;
        range.unwrapSelection(class_name, this.selectable);
    },
    ...
    deleteSelections: function(numclasses){
        for(var i=numclasses.length; i--;){
            ...
            var anchor = lastWithClass(this.selectable, numclass + ' ' + 'anchor');
...
Range.prototype.unwrapSelection = function(className, selectable){
    var elems = byClassName(selectable, className);

    for (var i=elems.length; i--;){
        var elem = elems[i];

        // if the element is an anchor, ignore it
        if (! hasClass(elem, 'anchor')) {
            // move child elements as siblings
            for (var j=elem.childNodes.length; j--;){
...
Range.prototype.addAnchor = function(anchor_span){
    this.splitBoundaries();
    var textNodes = this.getTextNodes();

    // Inserts anchor after the selected text nodes
    // if next sibling is null, it's still handled by default behaviour
    n = textNodes.length - 1;
    textNodes[n].parentNode.insertBefore(anchor_span, textNodes[n].nextSibling);
};
