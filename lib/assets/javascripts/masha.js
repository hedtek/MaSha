...
MaSha.prototype = {
    init: function(options, data){ // domready
        ...
        this.selection_index = this.selectable.id + '_' + this.options.selection_index;
    ...
    *
     */
    addSelection: function(range, page) {
        ...
        class_name = this.loadSelection(this.serializeRange(range), page);
    ...
    /*
     * Loads a serialized selection into MaSha without actually wrapping the selection
     *
     */
    loadSelection: function(range, page) {
        if (!page) page = this.currentPage;
        ...
        class_name = 'num'+this.counter;
        this.ranges[page][class_name] = range;
    ...
    *
    ...
    loadPage: function(page, firstLoad){
        if (!page) page = this.currentPage;
        ...
        var ranges = this.ranges[page];
        if (!ranges) return;

        for (var selectionClass in ranges){
            var range = ranges[selectionClass];

            range = this.deserializeRange(range);
            range = this.normalizeRange(range);
            range.wrapSelection(selectionClass+' user_selection_true');

            this.addAnchorForSelection(selectionClass);
        }
        return this.ranges[page];
    },
    ...
    deleteSelections: function(numclasses){
        for(var i=numclasses.length; i--;){
            ...
            for (var page in this.ranges){
                var ranges = this.ranges[page];

                if (ranges[numclass]) {
                   delete ranges[numclass];
                }
            }
        }
    },
    ...
    serializeRange: function(range) {
        ...
        if(this.options.validate){
            ...
            start += ':' + sums[0] + ':' + this.currentPage;
            end += ':' + sums[1]  + ':' + this.currentPage;
        }
        return start + "," + end;
    },
    ...
    checkSentence: function(range){
        ...
        if (nextAfterRange.match(/\.|\?|\!/)){
            ...
            if (range.startOffset == 0 &&
                ...
                range.startContainer.previousSibling.className == this.selection_index){
                return apply();
            }

            var node, iterator = range.getElementIterator();
            while ((node=iterator())) {
                if (node.nodeType == 1 && node.className == this.selection_index){
                    return apply();
                }
            }

            // The sentence is the first one in the paragraph
            if (this.isFirstTextNode(range.startContainer)){
                return apply();
            }
            ...
            for (var idx=0; idx<children.length; ++idx) {
                ...
                if (nodeType==3 && !child.nodeValue.match(this_.regexp)) {
                    ...
                    if (!blockStarted){
                        ...
                        index_span.id = this_.selection_index + MaSha.captureCount;
                        index_span.className = this_.selection_index;
    ...
    getFirstTextNode: function(numclass){
        ...
        var tnode = document.getElementById(this.selection_index+numclass);
    ...
    getNum: function(cont){
        while (cont.parentNode){
            while (cont.previousSibling){
                ...
                if (cont.nodeType == 1 && cont.className == this.selection_index){
                    return cont.id.replace(this.selection_index, '');
                }
            }
            cont = cont.parentNode;
        }
